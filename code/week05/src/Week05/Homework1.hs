...
mkPolicy pkh deadline () ctx = traceIfFalse "Incorrect signature" isSignedByCorrectKey &&
			       traceIfFalse "Deadline already passed" deadlineNotPassed
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    isSignedByCorrectKey :: Bool
    isSignedByCorrectKey = txSignedBy info $ unPaymentPubKeyHash pkh
